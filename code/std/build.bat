...
IF NOT EXIST c:\developer\Marching_squares\build\std mkdir c:\developer\Marching_squares\build\std
PUSHD c:\developer\Marching_squares\build\std

ECHO Removing all old files...
del /Q *.*

ECHO Building...
SET SourceFiles=../../code/std/*.cpp ../../code/DirectXRenderer.cpp
cl %CompilerOptions% %SourceFiles%  /link /SUBSYSTEM:windows %LinkerOptions% /out:Marching_squares_std.exe
...
mt.exe -nologo -manifest "../../data/a.manifest" -outputresource:"Marching_squares_std.exe;#1"
