...
SET LinkerLibs=user32.lib d3d10_1.lib dxgi.lib dxguid.lib dwrite.lib d2d1.lib
...
IF NOT EXIST c:\developer\Marching_squares\build\oop_std mkdir c:\developer\Marching_squares\build\oop_std
PUSHD c:\developer\Marching_squares\build\oop_std

ECHO Removing all old files...
del /Q *.*

ECHO Building...
SET SourceFiles=../../code/oop_std/*.cpp ../../code/DirectXRenderer.cpp
cl %CompilerOptions% %SourceFiles% /link /SUBSYSTEM:windows %LinkerOptions% /out:Marching_squares_oop_std.exe
...
mt.exe -nologo -manifest "../../data/a.manifest" -outputresource:"Marching_squares_oop_std.exe;#1"
